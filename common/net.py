...
class Decoder(chainer.Chain):
    def __init__(self, n_hidden=128, bottom_width=4, h_dim=512, ch=512, wscale=0.02,
                 z_distribution="uniform", hidden_activation=F.relu, output_activation=F.tanh, use_bn=True):
        ...
        with self.init_scope():
            w = chainer.initializers.Normal(wscale)
            self.lh0 = L.Linear(self.n_hidden, self.h_dim, initialW=w)
            self.lh1 = L.Linear(self.h_dim, self.h_dim, initialW=w)
            self.l0 = L.Linear(self.h_dim, bottom_width * bottom_width * ch,
                               initialW=w)
            self.dc1 = L.Deconvolution2D(ch, ch // 2, 4, 2, 1, initialW=w)
            self.dc2 = L.Deconvolution2D(ch // 2, ch // 4, 4, 2, 1, initialW=w)
            self.dc3 = L.Deconvolution2D(ch // 4, ch // 8, 4, 2, 1, initialW=w)
            self.dc4 = L.Deconvolution2D(ch // 8, 3, 3, 1, 1, initialW=w)
            if self.use_bn:
                self.bnh0 = L.BatchNormalization(h_dim)
                self.bnh1 = L.BatchNormalization(h_dim)
                self.bn0 = L.BatchNormalization(bottom_width * bottom_width * ch)
                self.bn1 = L.BatchNormalization(ch // 2)
                self.bn2 = L.BatchNormalization(ch // 4)
                self.bn3 = L.BatchNormalization(ch // 8)
    ...
    def __call__(self, z):
        if not self.use_bn:
            h = self.hidden_activation(self.lh0(z))
            h = self.hidden_activation(self.lh1(h))
            h = F.reshape(self.hidden_activation(self.l0(h)),
                          (len(z), self.ch, self.bottom_width, self.bottom_width))
            h = self.hidden_activation(self.dc1(h))
            h = self.hidden_activation(self.dc2(h))
            h = self.hidden_activation(self.dc3(h))
            ...
            h = self.hidden_activation(self.bnh0(self.lh0(z)))
            h = self.hidden_activation(self.bnh1(self.lh1(h)))
            h = F.reshape(self.hidden_activation(self.bn0(self.l0(h))),
                          (len(z), self.ch, self.bottom_width, self.bottom_width))
            h = self.hidden_activation(self.bn1(self.dc1(h)))
            h = self.hidden_activation(self.bn2(self.dc2(h)))
            h = self.hidden_activation(self.bn3(self.dc3(h)))
...
class Encoder(chainer.Chain):
    def __init__(self, bottom_width=4, ch=512, wscale=0.02, h_dim=512, output_dim=128, extra_mlp=False):
        w = chainer.initializers.Normal(wscale)
        ...
        with self.init_scope():
            ...
            self.c2_1 = L.Convolution2D(ch // 2, ch // 1, 4, 2, 1, initialW=w)
            self.c3_0 = L.Convolution2D(ch // 1, ch // 1, 3, 1, 1, initialW=w)
            self.l4 = L.Linear(bottom_width * bottom_width * ch, h_dim, initialW=w)
            self.l_mu = L.Linear(h_dim, output_dim, initialW=w)
            self.l_log_sigma = L.Linear(h_dim, output_dim, initialW=w)
            self.bn0_1 = L.BatchNormalization(ch // 4, use_gamma=False)
            self.bn1_0 = L.BatchNormalization(ch // 4, use_gamma=False)
            self.bn1_1 = L.BatchNormalization(ch // 2, use_gamma=False)
            self.bn2_0 = L.BatchNormalization(ch // 2, use_gamma=False)
            self.bn2_1 = L.BatchNormalization(ch // 1, use_gamma=False)
            self.bn3_0 = L.BatchNormalization(ch // 1, use_gamma=False)
            self.bn4 = L.BatchNormalization(h_dim, use_gamma=False)
    ...
    def __call__(self, x):
        ...
        return self.l_mu(h), self.l_log_sigma(h)

class Psi(chainer.Chain):
    def __init__(self, ch=512, wscale=0.02, input_dim=128, h_dim=64):
        w = chainer.initializers.Normal(wscale)
        super(Psi, self).__init__()
        with self.init_scope():
            self.l1 = L.Linear(input_dim, h_dim, initialW=w)
            self.l2 = L.Linear(h_dim, h_dim, initialW=w)
            self.l3 = L.Linear(h_dim, 1, initialW=w)
...
class CreateB(chainer.Chain):
    def __init__(self, wscale=0.02, input_dim=1):
        w = chainer.initializers.Normal(wscale)
        super(CreateB, self).__init__()
        with self.init_scope():
            self.l1 = L.Linear(input_dim, 1, initialW=w)
...
class WGANDiscriminator(chainer.Chain):
    ...
    #     feature = self.c3_0(self.h5)
    #     self.h6 = F.leaky_relu(feature)
    #     return feature, self.l4(self.h6)
    ...
    def __call__(self, x):
        self.x = x
        self.h0 = F.leaky_relu(self.c0(self.x))
        self.h1 = F.leaky_relu(self.bn1(self.c1(self.h0)))
        self.h2 = F.leaky_relu(self.bn2(self.c2(self.h1)))
        self.h3 = F.leaky_relu(self.bn3(self.c3(self.h2)))
        feature = self.bn4(self.c4(self.h3))
        self.h4 = F.leaky_relu(feature)
        return feature, self.l5(self.h4)
    

    def differentiable_backward(self, x):
        g = backward_linear(self.h4, x, self.l5)
        g = F.reshape(g, (x.shape[0], 512, 2, 2))
        g = backward_leaky_relu(self.h4, g, 0.2)
        g = backward_convolution(self.h3, g, self.c4)
        g = backward_leaky_relu(self.h3, g, 0.2)
